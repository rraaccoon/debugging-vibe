...
export const metadata: Metadata = { title: "출동 요청" };
    ...
    <PageTransition>
      <div className="stagger">
        <div style={stagger(0)} className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="font-display text-3xl leading-tight sm:text-4xl">출동 요청</h1>
            <p className="mt-3 max-w-prose text-ink-muted">
              매뉴얼에 없는 불은 여기에 신고하세요. 강사와 다른 팀이 출동해요. 진압되면 💧 를 눌러 주세요.
            </p>
          </div>
        ...
        <div style={stagger(1)} className="mt-6 flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="ml-auto tabular-nums text-ink-muted">{total}건</span>
        </div>

        {posts.length === 0 ? (
          <p style={stagger(2)} className="mt-14 text-center text-ink-muted">
            {total === 0 ? (
              "출동할 불이 없어요. 불나면 신고하세요."
            ) : (
              <>
                이 쪽에는 신고가 없어요.{" "}
          ...
          <ul style={stagger(2)} className="mt-4 flex flex-col gap-3">
            {posts.map((p) => {
              const s = STATUS[statusOf(p.reactions)];
              return (
                <li key={p.id}>
                  <Link
                    href={`/board/${p.id}`}
                    transitionTypes={["nav-forward"]}
                    className={`block rounded-2xl border border-line border-l-4 bg-white px-4 py-3.5 shadow-clay-sm transition-all duration-200 ease-out-expo hover:-translate-y-0.5 hover:shadow-clay ${s.bar}`}
                  >
                    <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-1">
                      ...
                      <span className="flex shrink-0 gap-1.5">
                        ...
                        <span
                          className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${p.answerCount ? "bg-action-soft text-action-deep" : "bg-line/70 text-ink-muted"}`}
                        >
                          {p.answerCount ? `출동 ${p.answerCount}` : "출동 없음"}
                        </span>
                      </span>
                    </div>
